...
const MESSAGE_TRUNCATION = '... (truncated)';

const capMessage = message =>
  message.length <= MAX_MESSAGE_LENGTH
    ? message
    : `${message.slice(0, MAX_MESSAGE_LENGTH - MESSAGE_TRUNCATION.length)}${MESSAGE_TRUNCATION}`;
...
export const isTransientNetworkError = error => {
  ...
  const statuses = chain.map(getStatus).filter(value => value != null);
  if (statuses.some(isTransientStatus)) return true;
...
export const describeNetworkError = error => {
  ...
  const labelled = flattenCauses(error).map(({ link, via }) => {
    ...
    const message = capMessage(String(link.message ?? link));
