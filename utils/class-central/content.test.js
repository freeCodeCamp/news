import { prepareContentForClassCentral, hashContent } from './content.js';

describe('Class Central content helpers:', () => {
  describe('prepareContentForClassCentral', () => {
    test('Returns the raw HTML unchanged when it fits within the length limit', () => {
      ...
      expect(prepareContentForClassCentral({ content: { html } })).toBe(html);
    });

    test('Returns null when the content is under 20 characters', () => {
      expect(
        prepareContentForClassCentral({ content: { html: '<p>Too short</p>' } })
      ).toBeNull();
    });

    test('Returns null when html is missing', () => {
      expect(prepareContentForClassCentral({})).toBeNull();
    });
    ...
    test('Combines the head and tail of long content, weighted 70/30', () => {
      ...
      const result = prepareContentForClassCentral({ content: { html } });
    ...
    test('Never cuts a trimmed section in the middle of a tag', () => {
      ...
      const [head, tail] = prepareContentForClassCentral({
        content: { html }
      }).split('\n');
